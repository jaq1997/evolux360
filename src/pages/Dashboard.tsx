// src/pages/Dashboard.tsx - VERSÃO FINAL COM AJUSTE NOS CARDS SUPERIORES
...
const DashboardHome = () => {
    ...
    const handleOrderCardClick = (orderData: Order) => setSelectedOrder(orderData);
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {/* O card "Taxa de conversão" foi removido */}
...
const Dashboard = () => {
    ...
    if (sessionLoading) {
        return <div className="min-h-screen bg-gray-50 flex items-center justify-center">Verificando sessão...</div>;
    }

    const getPageTitle = () => {
        const pathMap: { [key: string]: string } = { '/dashboard/vendas': 'Vendas', '/dashboard/crm': 'CRM', '/dashboard/estoque': 'Estoque', '/dashboard/financeiro': 'Financeiro' };
            ...
            <div className="min-h-screen flex w-full bg-gray-50">
                <Sidebar>
                    <SidebarContent>
                        ...
                        <SidebarGroup><SidebarGroupContent><SidebarMenu>{menuItems.map((item) => (<SidebarMenuItem key={item.title}><Link to={item.url} className={`flex items-center space-x-2 p-2 rounded-md ${location.pathname.startsWith(item.url) && item.url !== '/dashboard' || location.pathname === item.url ? 'bg-gray-100 text-[#5932EA] font-semibold' : 'hover:bg-gray-100/50'}`}><item.icon className="w-5 h-5" /><span>{item.title}</span></Link></SidebarMenuItem>))}</SidebarMenu></SidebarGroupContent></SidebarGroup>
                    </SidebarContent>
                </Sidebar>
                                ...
                                {session && (
                                    <div className="flex items-center space-x-4">
                                        <span className="text-sm text-gray-600">Olá, {session.user.email?.split('@')[0]}</span>
                                         <Button onClick={handleLogout} variant="outline" size="sm"><LogOut className="mr-2 h-4 w-4" />Sair</Button>
                                    </div>
                                )}
                            </div>
                        </header>
