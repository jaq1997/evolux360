// src/components/OrderDetailsModal.tsx - VERSÃO FINAL COM O BOTÃO REMOVIDO
...
// A timeline continua aqui, pois ainda é útil para visualização
...
export function OrderDetailsModal({ isOpen, onClose, order }: { isOpen: boolean, onClose: () => void, order: Order | null }) {
    ...
    const generateDynamicHistory = (currentStatus: OrderStatus | null) => {
        ...
        if (currentIndex === -1) {
            return [{
                status: currentStatus.replace(/_/g, ' ').replace(/^\w/, c => c.toUpperCase()),
                date: "Status atual",
                icon: CheckCircle,
                current: true,
            }];
    ...
    const dynamicHistory = generateDynamicHistory(order.status);
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-2xl bg-gray-50 max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6 p-1">
                    ...
                    {/* ✅ BOTÃO DE ATUALIZAR STATUS E TODA SUA LÓGICA FORAM REMOVIDOS */}
                            ...
                            <ul className="space-y-4">
                                {dynamicHistory.map((item, index) => (
                                    <li key={index} className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className={`font-semibold transition-colors duration-300 ${item.current ? 'text-gray-800' : 'text-gray-500'}`}>{item.status}</p>
